...
page = st.sidebar.selectbox("Select a Page", ["Water Network Designer", "INP File Upload","Pipe Criticality Analysis","Earthquake Simulation"])
...
#PAGE 3
# Page: Pipe Criticality Analysis
...
#earthquake simulation
    ...
    # Earthquake Simulation Page
    st.title("Earthquake Simulation for Water Network")

    # File upload section for INP file
    ...
    if uploaded_file is not None:
        ...
        if st.button("Run Earthquake Simulation"):
            ...
            wntr.graphics.plot_network(wn, link_attribute=R, node_size=0, link_cmap='viridis', title='Distance to Epicenter')
            st.pyplot()

            # Plot PGA
            st.subheader("Peak Ground Acceleration (PGA)")
            wntr.graphics.plot_network(wn, link_attribute=pga, node_size=0, link_cmap='viridis', link_width=1.5, title='Peak Ground Acceleration (PGA)')
            st.pyplot()

            # Plot PGV
            st.subheader("Peak Ground Velocity (PGV)")
            wntr.graphics.plot_network(wn, link_attribute=pgv, node_size=0, link_cmap='viridis', link_width=1.5, title='Peak Ground Velocity (PGV)')
            st.pyplot()
            ...
            wntr.graphics.plot_network(wn, link_attribute=pipe_Pr['Minor Leak'], node_size=0, link_cmap='viridis', link_range=[0, 1], link_width=1.5, title='Probability of Minor Leak')
            st.pyplot()

            # Plot Major Leak Probability
            wntr.graphics.plot_network(wn, link_attribute=pipe_Pr['Major Leak'], node_size=0, link_cmap='viridis', link_range=[0, 1], link_width=1.5, title='Probability of Major Leak')
            st.pyplot()
